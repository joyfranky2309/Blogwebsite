...
const getUserByUsername = async (req, res) => {
    try {
        const user = await UserCol.findOne({ username: req.params.username }, { password: 0 });
        if (!user) {
            return res.status(404).send("User not found");
        }
        res.status(200).json(user);
...
const updateUser = async (req, res) => {
    try {
        ...
        const user = await UserCol.findByIdAndUpdate(req.params.id, { $set: { preferences } }, { new: true });
        if (!user) {
            return res.status(404).send("User not found");
        }
...
module.exports = { registerUser, loginUser, getAllUsers, getUserByUsername, updateUser, deleteUser,getUserById };